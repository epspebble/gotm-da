...
!  turbulent length-scale
   double precision, public, dimension(:), allocatable   :: tke,eps,L

!  the turbulent diffusivities
   double precision, public, dimension(:), allocatable   :: num,nuh

!  the stability functions
   double precision, public, dimension(:), allocatable   :: cmue1,cmue2

!  long. Reynolds-stresses, temp.  variance, 
!  its rate of dissipation
   double precision, public, dimension(:), allocatable   :: uu,vv,ww,tt,chi

!  TKE at the old timestep
   double precision, public, dimension(:), allocatable   :: tkeo

!  alpha_M and alpha_N
   double precision, public, dimension(:), allocatable   :: as,an
 
!  the flux Richardson number
   double precision, public, dimension(:), allocatable   :: xRf 

!  some additional constants
   double precision, public          :: cm0,cmsf,cde,rcm, b1
  
!  Prandtl-number in neutrally stratified flow
   double precision, public          :: Prandtl0

!  parameters for wave-breaking 
   double precision, public          :: craig_m,sig_e0
...
!  the 'turb_param' namelist 
   double precision, public          :: cm0_fix=0.5477
   double precision, public          :: Prandtl0_fix=0.74
   double precision, public          :: cw=100.0
   logical                   :: compute_kappa=.false.
   double precision, public          :: kappa=0.4
   logical                   :: compute_c3=.false.
   double precision                  :: ri_st=0.25
   logical,  public          :: length_lim=.false.
   double precision, public          :: galp=0.53
   double precision, public          :: const_num=5.0e-4
   double precision, public          :: const_nuh=5.0e-4
   double precision, public          :: k_min=1.0e-8
   double precision, public          :: eps_min=1.0e-12

!  the 'generic' namelist 
   logical                   :: compute_param=.false.
   double precision, public          :: gen_m=1.5
   double precision, public          :: gen_n=-1.0
   double precision, public          :: gen_p=3.0
   double precision, public          :: cpsi1=1.44
   double precision, public          :: cpsi2=1.92
   double precision, public          :: cpsi3minus=0.0
   double precision, public          :: cpsi3plus=1.0
   double precision                  :: sig_kpsi=1.0
   double precision, public          :: sig_psi=1.3
   double precision                  :: gen_d=-1.2
   double precision                  :: gen_alpha=-2.0
   double precision                  :: gen_l=0.2

!  the 'keps' namelist
   double precision, public          :: ce1=1.44
   double precision, public          :: ce2=1.92
   double precision, public          :: ce3minus=0.0
   double precision, public          :: ce3plus=1.0
   double precision, public          :: sig_k=1.0
   double precision, public          :: sig_e=1.3
   logical,  public          :: sig_peps=.false.

!  the 'my' namelist
   double precision, public          :: e1=1.8
   double precision, public          :: e2=1.33
   double precision, public          :: e3=1.8
   double precision, public          :: sq=0.2
   double precision, public          :: sl=0.2
   integer,  public          :: my_length=1
   logical,  public          :: new_constr=.false.
 
!  the 'iw' namelist
   integer                   :: iw_model=0
   double precision, public          :: alpha=0.0
...
81 write(0,*) 'FATAL ERROR: ', 'I could not read "turbulence" namelist'
   stop 'init_turbulence'
82 write(0,*) 'FATAL ERROR: ', 'I could not read "bc" namelist'
   stop 'init_turbulence'
83 write(0,*) 'FATAL ERROR: ', 'I could not read "turb_param" namelist'
   stop 'init_turbulence'
84 write(0,*) 'FATAL ERROR: ', 'I could not read "generic" namelist'
   stop 'init_turbulence'
85 write(0,*) 'FATAL ERROR: ', 'I could not read "keps" namelist'
   stop 'init_turbulence'
86 write(0,*) 'FATAL ERROR: ', 'I could not read "my" namelist'
   stop 'init_turbulence'
87 write(0,*) 'FATAL ERROR: ', 'I could not read "iw" namelist'
   stop 'init_turbulence'
...
! do some checks
   ...
   if (tke_method.ne.tke_keps) then
      write(0,*) 'The generic scale equation should be used only'
      write(0,*) 'in connection with the dynamic equation for'
      write(0,*) 'the tke (use tke_method = 2).'
      write(0,*) 'Please change gotmturb.inp accordingly'
      write(0,*) 'Program aborts now in turbulence.F90'
      stop
   endif

   if (gen_d.gt.0.) then
      write(0,*) 'Temporal decay rate d in homogeneous'
      write(0,*) 'turbulence has to be negative.'
      write(0,*) 'Measured values are: -1.0 < d < -1.3'
      write(0,*) 'Please change gotmturb.inp accordingly'
      write(0,*) 'Program aborts now in turbulence.F90'
      stop
   endif
   ...
   if (gen_alpha.gt.0.) then
      write(0,*) 'Decay exponent alpha has to be negative.'
      write(0,*) 'Measured values are: -3 < alpha < -2' 
      write(0,*) 'Please change gotmturb.inp accordingly'
      write(0,*) 'Program aborts now in turbulence.F90'
      stop
   endif
   ...
   if (gen_l.lt.0.) then
      write(0,*) 'Slope L of the length scale in shear-free'
      write(0,*) 'turbulence has to be positive.'
      write(0,*) 'Measured values are: 0.15 < L < 0.25' 
      write(0,*) 'Please change gotmturb.inp accordingly'
      write(0,*) 'Program aborts now in turbulence.F90'
      stop
   endif
...
! check for negative radicand and compute n
   if (rad.gt.0) then
      ...
      write(0,*) 'Negative radicand discovered in computing'
      write(0,*) 'exponent n of the generic model.'
      write(0,*) 'Please choose other value for gen_m in gotmturb.inp'
      write(0,*) 'Program aborts now in turbulence.F90'
      stop
   endif
...
! check for positive exponent n   
   if (gen_n.gt.0.) then
      write(0,*) 'A positive exponent n of the lengt scale l'
      write(0,*) 'has been computed. This indicates that the model'
      write(0,*) 'would require a wall-function.'
      write(0,*) 'Please change gotmturb.inp accordingly'
      write(0,*) 'Program aborts now in turbulence.F90'
      stop
   endif
...
! check for negative Schmidt-number sig_psi
   if (sig_psi.lt.0.) then
      write(0,*) 'A negative Schmidt-number sig_psi has been'
      write(0,*) 'computed. Physically, that does not make sense.'
      write(0,*) 'Possible reason: You set gen_d < -2 in gotmturb.inp?'
      write(0,*) 'Please change gotmturb.inp accordingly'
      write(0,*) 'Program aborts now in turbulence.F90'
      stop
   endif
...
! compute kappa from the parameters
      if (compute_kappa)  then
         ...
         if (rad .gt. 0) then
            ...
            write(0,*) 'Negative radicand discovered in computing'
            write(0,*) 'kappa for the k-epsilon model.'
            write(0,*) 'Possible reason: you took ce2 < ce1 '
            write(0,*) 'Please change gotmturb.inp accordingly.'
            write(0,*) 'Program aborts now in turbulence.F90'
            stop
         endif
         if (sig_peps) then
            write(0,*) 'For using the Craig & Banner 1994 parameterisation'
            write(0,*) 'by Burchard (2001) kappa must be prescribed.'
            write(0,*) 'For doing so, compute_kappa=.false. must be set.'
            write(0,*) 'Please change gotmturb.inp accordingly.'
            write(0,*) 'Program aborts now in turbulence.F90'
            stop
         end if
...
! compute c3 from given steady-state Richardson-number, or vice-versa
   ...
   case(generic_eq)
      ...
      if (rad.gt.0) then
         ...
         write(0,*) 'Negative radicand discovered in computing'
         write(0,*) 'kappa for the generic model.'
         write(0,*) 'Possible reason: you took cpsi2 < cpsi1'
         write(0,*) 'Please change gotmturb.inp accordingly.'
         write(0,*) 'Program aborts now in turbulence.F90'
         stop
      endif
...
! compute kappa from the parameters or vice-versa
      if (compute_kappa)  then
         ...
         if (rad.gt.0) then
            ...
            write(0,*) 'Negative radicand discovered in computing'
            write(0,*) 'kappa for the Mellor-Yamada model.'
            write(0,*) 'Possible reason: you took E2 < E1-1 '
            write(0,*) 'Please change gotmturb.inp accordingly.'
            write(0,*) 'Program aborts now in turbulence.F90'
            stop
         endif
...
! Report on the properties of each model 
   select case(len_scale_method)
      case(Parabola)
         write(0,*) '       ', ' '
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using a one-equation model'
         write(0,*) '       ', 'with a parabolic prescribed length-scale.'
         write(0,*) '       ', 'The properties of this model are:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'Schmidt-number for k,          sig_k =', sig_k
         write(0,*) '           ', 'von Karman constant,           kappa =', kappa
         write(0,*) '       ', ' '

         write(0,*) '           ', 'Value of the stability function'       
         write(0,*) '           ', 'in the log-law,                   cm0 =', cm0
         write(0,*) '           ', 'in shear-free turbulence,        cmsf =', cmsf
         write(0,*) '       ', ' '
         write(0,*) '           ', 'At the surface:'
         write(0,*) '           ', 'spatial decay rate (no shear), alpha =', gen_alpha
         write(0,*) '           ', 'length-scale slope (no shear), L     =', gen_l
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', ' '
      case(Triangle)
         write(0,*) '       ', ' '
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using a one-equation model'
         write(0,*) '       ', 'with a triangular prescribed length-scale.'
         write(0,*) '       ', 'The properties of this model are:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'Schmidt-number for k,          sig_k =', sig_k
         write(0,*) '           ', 'von Karman constant,           kappa =', kappa
      ...
      case(Xing)
         ...
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using a one-equation model'
         write(0,*) '       ', 'with the prescribed length-scale of Xing and Davies (1995)'
         write(0,*) '       ', 'The properties of this model are:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'Schmidt-number for k,          sig_k =', sig_k
         write(0,*) '           ', 'von Karman constant,           kappa =', kappa
      ...
      case(RobertOuellet)
         ...
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using a one-equation model'
         write(0,*) '       ', 'with the prescribed length-scale of Robert and Ouellet (1987)'
         write(0,*) '       ', ' '
         write(0,*) '       ', 'The properties of this model are:'
         write(0,*) '           ', 'Schmidt-number for k,          sig_k =', sig_k
         write(0,*) '           ', 'von Karman constant,           kappa =', kappa
      ...
      case(Blackadar)
         ...
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using a one-equation model'
         write(0,*) '       ', 'with the length-scale of Blackadar (1962)'
         write(0,*) '       ', 'The properties of this model are:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'Schmidt-number for k,          sig_k =', sig_k
         write(0,*) '           ', 'von Karman constant,           kappa =', kappa
      ...
      case(BougeaultAndre)
         ...
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using a one-equation model'
         write(0,*) '       ', 'with the length-scale of Bougeault and Andre (1986)'
         write(0,*) '       ', 'The properties of this model are:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'Schmidt-number for k,          sig_k =', sig_k
         write(0,*) '           ', 'von Karman constant,           kappa =', kappa
      ...
      case(ispra_length)
         write(0,*) '       ', ' '
         write(0,*) '       ', 'You are using a one-equation model'
         write(0,*) '       ', 'with the ISPRAMIX length-scale (see GOTM-report)'
         write(0,*) '       ', 'The properties of this model are:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'Schmidt-number for k,          sig_k =', sig_k
         write(0,*) '           ', 'von Karman constant,          kappa  =', kappa
      ...
      case(diss_eq)
         write(0,*) '       ', ' '
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using the k-epsilon model'
         write(0,*) '       ', 'with the following properties:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'ce1                                  =', ce1
         write(0,*) '           ', 'ce2                                  =', ce2
         write(0,*) '           ', 'ce3minus                             =', ce3minus
         write(0,*) '           ', 'ce3plus                              =', ce3plus
         write(0,*) '           ', 'sig_k                                =', sig_k
         write(0,*) '           ', 'sig_e                                =', sig_e
         write(0,*) '       ', ' '
         write(0,*) '           ', 'Value of the stability function'       
         ...
         write(0,*) '       ', ' '
         write(0,*) '           ', 'von Karman constant,           kappa =', kappa
         write(0,*) '           ', 'homogeneous decay rate,            d =', gen_d
         write(0,*) '           ', 'spatial decay rate (no shear), alpha =', gen_alpha
         write(0,*) '           ', 'length-scale slope (no shear),     L =', gen_l
         write(0,*) '           ', 'steady-state Richardson-number, Ri_st=', ri_st
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', ' '
      case(length_eq)
         write(0,*) '       ', ' '
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using the Mellor-Yamada model'
         write(0,*) '       ', 'with the following properties:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'B1                                   =', b1
         write(0,*) '           ', 'E1                                   =', e1
         ...
         write(0,*) '           ', 'steady-state Richardson-number, Ri_st=', ri_st
         write(0,*) '       ', ' '
         write(0,*) '           ', 'At the surface (i.e. with wall-function):'
         write(0,*) '           ', 'spatial decay rate (no shear), alpha =', gen_alpha
         write(0,*) '           ', 'length-scale slope (no shear),     L =', gen_l
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', ' '
      case(generic_eq)
         write(0,*) '       ', ' '
         write(0,*) '       ', '--------------------------------------------------------'
         write(0,*) '       ', 'You are using the generic two-equation model'
         write(0,*) '       ', 'with the following properties:'
         write(0,*) '       ', ' '
         write(0,*) '           ', 'exponent of k in psi-equation,    m  =', gen_m
         write(0,*) '           ', 'exponent of l in psi-equation,    n  =', gen_n
         write(0,*) '           ', 'exponent of cm0 in psi-equation,  p  =', gen_p
         write(0,*) '           ', 'cpsi1                                =', cpsi1
         write(0,*) '           ', 'cpsi2                                =', cpsi2
         write(0,*) '           ', 'cpsi3minus                           =', cpsi3minus
         write(0,*) '           ', 'cpsi3plus                            =', cpsi3plus
         write(0,*) '           ', 'sig_k                                =', sig_kpsi
         write(0,*) '           ', 'sig_psi                              =', sig_psi
         write(0,*) '       ', ' '
         write(0,*) '           ', 'Value of the stability function'       
         write(0,*) '           ', 'in the log-law,                   cm0 =', cm0
         write(0,*) '           ', 'in shear-free turbulence,        cmsf =', cmsf
...
! !INPUT PARAMETERS:
   ...
   double precision, intent(in)                :: u_taus,u_taub,z0s,z0b
   double precision, intent(in)                :: h(0:nlev)
   double precision, intent(in)                :: P(0:nlev),B(0:nlev)
   double precision, intent(in)                :: NN(0:nlev),SS(0:nlev)
!
...
! !INTERFACE:
   ...
   double precision function k_bc(bc,type,zi,z0,u_tau)
!
!
! !DESCRIPTION:
...
! !INPUT PARAMETERS:
   ...
   double precision, intent(in)                :: zi,z0,u_tau
!
! !REVISION HISTORY:
!  Original author(s): Lars Umlauf
!
...
! !LOCAL VARIABLES:
   ...
   double precision, external        :: fk_craig
!
!-----------------------------------------------------------------------
!BOC
! check for correct function call
   ...
   select case(type)
      ...
      case(viscous)
         write(0,*) 'Sorry, viscous boundary layers not yet implemented.'
         write(0,*) 'Please change gotmturb.inp accordingly'
         write(0,*) 'Program aborts now in turbulence.F90'
         stop
...
!
! !INTERFACE:
   double precision function  q2over2_bc(bc,type,zi,z0,u_tau)
!
!
! !DESCRIPTION:
! Computes prescribed and flux boundary conditions for  the transport
...
!
! !INTERFACE:
   double precision function  epsilon_bc(bc,type,zi,ki,z0,u_tau)
!
!
!DESCRIPTION:
! Computes prescribed and flux boundary conditions for  the transport
...
! !INPUT PARAMETERS:
   integer, intent(in)                 :: bc,type
   double precision, intent(in)                :: zi,ki,z0,u_tau
!
! !REVISION HISTORY:
!  Original author(s): Lars Umlauf
!
...
!
! !INTERFACE:
   double precision function  psi_bc(bc,type,zi,ki,z0,u_tau)
!
!DESCRIPTION:
! Computes prescribed and flux boundary conditions for  the transport
...
!
! !INTERFACE:
   double precision function  q2l_bc(bc,type,zi,ki,z0,u_tau)
!
!
! !DESCRIPTION:
! Computes prescribed and flux boundary conditions for  the transport
...
! !LOCAL VARIABLES:
     ...
     double precision                :: fc,fp,step,e=1.e-10,cme
!
!-----------------------------------------------------------------------
!BOC
   an=0.
...
!
! !INTERFACE:
   double precision function c3(c1,c2,Ri)
!
! !DESCRIPTION:
...
!
! !INPUT PARAMETERS:
   double precision, intent(in)                :: c1,c2,Ri
!
! !REVISION HISTORY:
!  Original author(s): Lars Umlauf, Hans Burchard
!
...
! !LOCAL VARIABLES:
     integer                       :: i,imax=100
     double precision                      :: fc,fp,e=1.e-8,step,ann
     double precision                      :: NN(0:2),SS(0:2)
!
!-----------------------------------------------------------------------
!BOC
   ...
   do i=0,imax
      ...
      if (abs(step).gt.100.) then
         write(0,*) 'Method for calculating c3 does not converge.'
         write(0,*) 'Probably, the prescribed steady-state Richardson number' 
...
!
! !INTERFACE:
   double precision function  Rist(c1,c2,c3)
!
! !DESCRIPTION:
