...
! !INPUT PARAMETERS:
   ...
   double precision, intent(in)                :: NN(0:nlev),SS(0:nlev),h(0:nlev)
   double precision, intent(in)                :: depth,dt
!
! !INPUT/OUTPUT PARAMETERS:
   double precision                            :: ga(0:nlev)
!
! !REVISION HISTORY:
...
! !LOCAL VARIABLES:
   integer                   :: i,j,split
   double precision                  :: ganew(0:nlev),gaold(0:nlev)
   double precision                  :: NNloc(0:nlev),SSloc(0:nlev)
   double precision                  :: rat,center,g=9.81
   logical,save              :: first=.true.
   double precision,dimension(0:nlev):: av1,av2,av3,av
!
!-----------------------------------------------------------------------
!BOC
  if (abs(dt/dtgrid-nint(dt/dtgrid)).gt.1.e-10) then
     write(0,*) 'dt/dtgrid must be integer number.' 
     write(0,*) 'Please adjust dt or dtgrid accordingly.'
     write(0,*) ' Program aborted in adaptivegrid.'
     stop 
  end if
