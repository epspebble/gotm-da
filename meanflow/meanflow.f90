...
!  the 'meanflow' namelist
   ...
   double precision, public                    :: cp=4000.     !3985.
...
!
! !DEFINED PARAMETERS:
   double precision, public, parameter         :: pi=3.14159265358979
!
! !REVISION HISTORY:
...
! !LOCAL VARIABLES:
   integer                   :: rc
   DOUBLE PRECISION:: gamma,c1,c2,c3,c4,phi    !SP numbers need to calculate g

   namelist /meanflow/  h0b,z0s_min,charnok,charnok_val,ddu,ddl,       &
                        grid_method,c1ad,c2ad,c3ad,c4ad,Tgrid,NNnorm,  &
                        SSnorm,dsurf,dtgrid,grid_file,rho_0,cp,&
...
!SP 23/01/06 calculate gravity constant from latitude
gamma=9.7803267715
c1=0.0052790414
c2=0.0000232718
c3=0.0000001262
...
gravity=gamma*(1.0+c1*((sin(phi))**2)+c2*((sin(phi))**4)+ &
	c3*((sin(phi))**6)+c4*((sin(phi))**8))
