...
def timestr(nctime,i):
    ...
    from datetime import strftime, num2date
    return strftime(num2date(nctime[i],nctime.units),'%Y-%m-%d %H:%M:%S')        
...
def prepare_engine():
    ...
    dv.execute("sys.path.append(os.path.join(userhome,project_folder,'python'))")
    dv.execute('os.chdir("{}")'.format(base_folder))
    dv.execute('from gotmks import *')
    dv.execute('from gotm_medsea import *')
...
def core_dat(year,month,m,n,
             nc_dict={dat_fn: None for dat_fn in ['tprof','sprof','heat','meteo','sst','chlo']}):
    """
        ...
        with open(fn) as f:
            ...
            if dat_fn == 'tprof':
                for i in range(len(time)):
                    ...
                    for j in range(18):
                        line = print(-nc['depth'][j],nc['votemper'][i,j,m,n])
                        f.write(line)
            elif dat_fn == 'sprof':
                for i in range(len(time)):
                    f.write(timestr(time,i) + ' 18 2\n') # Always 18 readings and two columns.
                    for j in range(18):
                        line = print(-nc['depth'][j],nc['vosaline'][i,j,m,n])
                        f.write(line)
            elif dat_fn == 'heat':
                ...
                for i in range(len(time)):
                    ...
                    col[8] = nc['snow'][i,m,n]
                    line = print(*col) # Use the default printing conventions for the respective data type.
                    f.write(line)
            elif dat_fn == 'sst':
                for i in range(len(time)):
                    line = print(timestr(time,i),nc['analysed_sst'][i,m,n])
                    f.write(line)
            elif dat_fn == 'chlo':
                for i in range(len(time)):
                    line = print(timestr(time,i),nc['chlor_a'][i,m,n])
                    f.write(line)
            else:
...
def medsea_dat(engine=None, year=2014, month=1):
    ...
    data_sources = \
    {'heat' : Dataset(os.path.join(base_folder,'forcing', 'medsea_ERA_heat_{:d}{:02d}.nc'.format(year,month)),'r'),
     'met'  : Dataset(os.path.join(base_folder,'forcing', 'medsea_ERA_meteo_{:d}{:02d}.nc'.format(year,month)),'r'),
     'tprof': Dataset(os.path.join(base_folder,'profiles', 'medsea_rea_votemper_{:d}{:02d}.nc'.format(year,month)),'r'),
     'sprof': Dataset(os.path.join(base_folder,'profiles', 'medsea_rea_vosaline_{:d}{:02d}.nc'.format(year,month)),'r'),
     'sst'  : Dataset(os.path.join(base_folder,'forcing', 'medsea_OSTIA_sst_{:d}{:02d}.nc'.format(year,month)),'r'),
     'chlo' : Dataset(os.path.join(base_folder,'forcing', 'medsea_OSTIA_sst_{:d}{:02d}.nc'.format(year,month)),'r')}
    
    results = engine.map(core_dat,
                         itertools.repeat(year,21*57),
                         itertools.repeat(month,21*57),
                         mm,nn,
                         nc_dict = itertools.repeat(data_sources,21*57))
