...
!
! !LOCAL VARIABLES:
  double precision  :: dummy, I_0_down,qb_down,lwrcloud,top,bottom
!  double precision  :: swr_error(1:120)
...
!      The heat fluxes
      ...
      select case (flux_method)
         case (CONSTVAL)
            heat=const_qout
         case (FROMFILE)
             call read_heat_flux(jul,secs,I_0_down,dummy,qb_down)
         case default
      end select
      ...
      select case (swr_method)
         ...
         case (FROMFILE)
            if (flux_method .ne. FROMFILE ) then
              ...
              I_0 = I_0_down*(1.-albedo)
            end if 
         case default
      end select

!SP represents errors in swr obs
              I_0=(1+swr_error)*I_0
...
!     The momentum fluxes
     ...
     !SP this represents possible errors in obs 14/12/05
     !       tx=(1+wind_error)*tx
     !       ty=(1+wind_error)*ty
     !SP
...
!   count1=720
!   count2=count2+1
!   OPEN(UNIT=91,FILE="/users/spi/gotmlink/OBS/COARE/DEC05/alpha.dat")
!   DO k=1,count2
!      READ(91,*) alpha(k)
!   END DO
!   CLOSE(UNIT=91)
!end if
!   qshort  = qtot*(1.-albedo)*alpha(count2)
!   PRINT*,alpha(count2)
!   count1=count1-1
!SP  14/12/05
...
!SP 14/10/05 read in state vector 2
         if (loop_counter==1) then
            OPEN(UNIT=74,FILE="current_optimal_state2.dat",IOSTAT=ios)
            IF(ios/=0) THEN
               PRINT*,"Error during opening input file, stopping"; STOP
            END IF
            READ (UNIT=74,*) qe2
...
!SP 14/10/05 end read in state vector 2
...
!SP 14/10/05 print out state vector 2
     if (loop_counter==1440) then
        OPEN(UNIT=74,FILE="end_state2.dat",IOSTAT=ios)
	IF(ios/=0) THEN
		PRINT*,"Error during opening input file, stopping"; STOP
       	END IF
        WRITE (UNIT=74,*) qe2
...
!SP 14/10/05 end print out state vector 2
   return
