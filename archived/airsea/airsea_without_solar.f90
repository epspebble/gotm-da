...
module airsea
  ...
  !WT 20171102 Moved to solar.f90. Also, we are replacing all occurences of I_0_calc by qtot, which
  ! represents the total downward SWR just before reaching the air-sea interface.
  !double precision, public	:: I_0_calc,coszen
  !double precision, public      :: albedo,qtot,sunbet

  !WT 20171102 The following are used for calling temperature()
  double precision, public      :: cosr,coszen,sunbet,qdir_frac,qdiff_frac

  !WT 20171102 Put back for use in this subroutine and for output.
  ! NOTE: we are replacing all occurences of I_0_calc by qtot. 
  double precision, public      :: qtot, albedo
...
!!!!! SH - 14/08/2003 - additional parameters for thermal skin etc 
  ...
  double precision, public	:: sssT=0., delta=0 ! Removed I_0_cs, but rely on qtot*(1-albedo)
...
902 write(0,*) 'WARNING: ', 'I could not read coolskin namelist.\n Resorting to default values.'
  ...
  subroutine air_sea_interaction(jul,secs)
    ...
    !WT 20171102, many values, including coszen, albedo,...
    ! written module public variables. Check the comments in solar.f90 
    call short_wave_radiation(jul,secs,alat,alon,qtot)
    ...
    case (FROMFILE)
       if (flux_method .ne. FROMFILE ) then
          ...
          !WT 20171102 The above question has been settled with SP. We should leave it as it is,
          ! even if it is slightly above 1, so that the output net SWR with the albedo put back
          ! in would give the same value as from the data from ERA-INTERIM.

          
          ! I_0=cloud_factor*I_0_calc  ! cloud_factor comes from heat.dat 2nd column

          !WT 20171102 I_0 means the net swr BEFORE taking out the albedo factor.
          ! Therefore, the net swr should instead of I_0*(1-albedo).
          I_0=cloud_factor*qtot*(1-albedo)  ! cloud_factor comes from heat.dat 2nd column
...
!!!!! SH - 11/08/2003 - removed calculated short wave from this routine - not needed
     ...
     subroutine integrated_fluxes(dt,int_cs)
       ...
       int_cs = int_cs + dt*qtot*(1-albedo) !WT I_0_cs = qtot*(1-albedo)
