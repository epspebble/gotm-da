...
!     The heat fluxes
      ...
      select case (flux_method)
         case (CONSTVAL)
            heat=const_qout
         case (FROMFILE)
             call read_heat_flux(jul,secs,I_0_down,heat,qb_down)
         case default
      end select
...
!SP - A file to read in swr errors so that swr obs can be adjusted 29/07/05              
!if(count==1) then
!   OPEN(UNIT=79,FILE="OBS/COARE/swr_error.dat",IOSTAT=ios)
!		IF(ios/=0) THEN
!			PRINT*,"Error during opening input file, stopping"; STOP
!		END IF
!	DO i=1,120               	
!		READ (UNIT=79,FMT='(2X,E14.11)')  swr_error(i)
!	END DO
!   CLOSE (UNIT=79)
!   count=count+1
!end if
...
! !LOCAL VARIABLES:
   ...
   integer                           ::i,ios,count=1,count2=1,count3=0
!   double precision                  ::wind_error(1:120)
!
!
!-----------------------------------------------------------------------
!BOC
...
!SP - A file to read in wind errors so that wind obs can be adjusted 29/07/05  !if(count==1) then
!   OPEN(UNIT=78,FILE="OBS/COARE/wind_error.dat",IOSTAT=ios)
!		IF(ios/=0) THEN
!			PRINT*,"Error during opening input file, stopping"; STOP
!		END IF
!	DO i=1,120               	
!		READ (UNIT=78,FMT='(2X,E14.11)')  wind_error(i)
!	END DO
!   CLOSE (UNIT=78)
!   count=count+1
!end if
