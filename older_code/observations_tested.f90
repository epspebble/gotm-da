...
! SH - 03/12/2003
!  'observed' smoothed temperature profile
   double precision, public, dimension(:), allocatable   :: smootprof
! SH - 03/12/2003
!  'observed' dummy temperature profile
   double precision, public, dimension(:), allocatable   :: tprofdum
...
! SH Helen's relaxation time at night - here (30/10/2003)
   double precision, public, dimension(:), allocatable   :: TRelaxTauNight
...
!  Clear water - Defant,A. 1961. - SH 07/11/02!
   ...
   double precision, public, dimension(9) :: &
     zdeta = (/3.48e1,2.27,3.15e-2,5.48e-3,8.32e-4 &
...
!SH - allow for a time smoothed temperature profile series to be read.
!SH 03/12/2003
   integer, public                   :: smoo_t_prof_method=0
   character(LEN=255)                :: smoo_t_prof_file='smootprof.dat'
   double precision, public          :: TRelaxSmooLevel=0.
   double precision                  :: z_t1,t_1,z_t2,t_2
   double precision, public          :: TRelaxTauM=0.
   double precision, public          :: TRelaxTauS=0.
   double precision, public          :: TRelaxTauB=0.
   double precision, public          :: TRelaxSurf=0.
   double precision, public          :: TRelaxBott=0.
!HK add switch for night time relaxation
   logical, public                   :: TRelax_night=.false.
   double precision, public          :: TRelaxTauNightM=0.
   double precision, public          :: TRelaxTauNightS=0.
   double precision, public          :: TRelaxTauNightB=0.
...
!  Light extinction - the 'extinct' namelist
   ...
   integer, public      :: skin_method=1  !HK
...
!   integer, public           :: advection_method=0
...
! SH - 03/12/2003
   integer, parameter        :: smoo_t_prof_unit=41
...
!
!  $Log: observations_tested.f90,v $
!Revision 1.3  2003/12/03  16:57:03  stephenh
!Added smoothed temperature profile from file
!SH 03/12/2003
!
!Revision 1.2  2003/10/27  15:35:19  helen
!new namelist terms to allow for nighttime relaxation of T profile
!
...
!                       TRelaxTauM,TRelaxTauB,TRelaxTauS,TRelaxBott, &
!                       TRelaxSurf,TRelax_night,TRelaxTauNightM,     &
!                       TRelaxTauNightS,TRelaxTauNightB
!SH 03/12/2003
   namelist /tprofile/ t_prof_method,smoo_t_prof_method,z_t1,t_1,z_t2,t_2, &
                       t_prof_file, smoo_t_prof_file,TRelaxSmooLevel, &
                       TRelaxTauM,TRelaxTauB,TRelaxTauS,TRelaxBott, &
                       TRelaxSurf,TRelax_night,TRelaxTauNightM,     &
                       TRelaxTauNightS,TRelaxTauNightB
   ...
   namelist /extinct/ extinct_method,extinct_file, skin_method !HK
   namelist /w_advspec/                                         &
            w_adv_method,w_adv0,w_adv_file,w_adv_discr                      
   namelist /zetaspec/                                          &
...
! SH 03/12/2003
!   read(namlst,nml=smootprofile,err=79)
...
! SH 03/12/2003
   allocate(smootprof(0:nlev),stat=rc)
   if (rc /= 0) STOP 'init_observations: Error allocating (smootprof)'
   smootprof = 0.

   allocate(tprofdum(0:nlev),stat=rc)
   if (rc /= 0) STOP 'init_observations: Error allocating (tprofdum)'
   tprofdum = 0.
...
! SH add Helen's night relaxation here
   allocate(TRelaxTauNight(0:nlev),stat=rc)
   if (rc /= 0) STOP 'init_observations: Error allocating (TRelaxTauNight)'
   TRelaxTauNight = 0.
...
!SH 03/12/2003  TRelaxSmooLevel : level at & below which relaxation for T 
!               occurs to a smoothed profile (in meters)
...
! SH Helen's night relaxion defined
      TRelaxTauNight(i)=TRelaxTauNightM
...
! SH Helen's night relaxion defined
      if (db.le.TRelaxBott) TRelaxTauNight(i)=TRelaxTauNightB
      if (ds.le.TRelaxSurf) TRelaxTauNight(i)=TRelaxTauNightS
      db=db+0.5*h(i)
      ds=ds-0.5*h(i)
      ...
      if ((t_prof_method.ne.0).and. TRelax_night .and.(TRelaxTauNight(i).le.0.)) then
         write(0,*) '       ', ''
         write(0,*) '       ', '********************************************'
         write(0,*) '       ', 'TRelaxTauNight at i=',i,' is not a positive value.'
         write(0,*) '       ', 'Please correct namelist.inp and rerun.'
         write(0,*) '       ', 'Program aborted.'
         write(0,*) '       ', '********************************************'
         stop 'init_observations'
      end if
      if ((smoo_t_prof_method.ne.0).and.(TRelaxTau(i).le.0.)) then
         write(0,*) '       ', ''
         write(0,*) '       ', '********************************************'
         write(0,*) '       ', 'TRelaxTau at i=',i,' is not a positive value.'
         write(0,*) '       ', 'Please correct namelist.inp and rerun.'
...
!  The smoothed temperature profile
   select case (smoo_t_prof_method)
      case (NOTHING)
         smootprof = 0.
      case (ANALYTICAL)
         call analytical_profile(nlev,z,z_t1,t_1,z_t2,t_2,smootprof)
      case (FROMFILE)
         open(smoo_t_prof_unit,file=smoo_t_prof_file,status='unknown',err=110)
         write(0,*) '       ', 'Reading temperature profiles from:'
         write(0,*) '           ', trim(smoo_t_prof_file)
         call get_smoo_t_profile(smoo_t_prof_unit,julday,secs,nlev,z)
! SH 03/12/2003 initialise tprofdum
         tprofdum=smootprof
      case default
   end select
...
! SH 03/12/2003
!79 write(0,*) 'FATAL ERROR: ', 'I could not read "smootprofile" namelist'
!   stop 'init_observations'
...
109 write(0,*) 'FATAL ERROR: ', 'Unable to open "',trim(e_prof_file),'" for reading'
   stop 'init_observations'
! SH 03/12/2003
110 write(0,*) 'FATAL ERROR: ', 'Unable to open "',trim(smoo_t_prof_file),'" for reading'
   stop 'init_observations'
...
!
! !INTERFACE:
   subroutine get_all_obs(julday,secs,nlev,z,h,depth)
!
! !DESCRIPTION:
...
! !INPUT PARAMETERS:
   integer, intent(in)                 :: julday,secs
   integer, intent(in)                 :: nlev
   double precision, intent(in)                :: z(:),h(:)
! SH 03/12/2003
   double precision, intent(in)                :: depth
!  !LOCAL VARIABLES:
   double precision                  :: ds
...
! SH 03/12/2003
   if(smoo_t_prof_method .eq. 2) then
      call get_smoo_t_profile(smoo_t_prof_unit,julday,secs,nlev,z)
      tprofdum=smootprof
      ds=depth
      do i=1,nlev
         ds=ds-0.5*h(i)
! SH 03/12/2003 - assign tprof to tprofdum above TRelaxSmooLevel
! SH 03/12/2003 - assign smootprof to tprofdum below TRelaxSmooLevel
         if (ds.le.TRelaxSmooLevel) tprofdum(i)=tprof(i)
         ds=ds-0.5*h(i)
      end do
   end if
