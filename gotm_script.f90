...
!HK 
   ...
   use airsea, only                : ostia,ostia_diff,ostia_sq_diff
...
!  open the namelist file.
   ...
   read(start,'(i4,a1,i2,a1,i2)')  &
                          stuff,stuffb,stuff,stuffb,day
...
!SP 17/10/05 evaluate cost function
      ...
      IF((ABS(swr_error)-0.0).LT.0.0001) THEN
      ELSE
         swr_error_total=swr_error
      END IF
!every 24 hours
      if (day_cycle==2880) then
		...
		PRINT*,"Error during opening input file, stopping"; STOP
       	 END IF
         WRITE (75,*) seviri_diff+amsre_diff+tmi_diff,cloud,&
              seviri_sq_diff+amsre_sq_diff+tmi_sq_diff,&
              swr_error_total,wind_error,day
         CLOSE (UNIT=75)
      end if
