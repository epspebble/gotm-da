...
! !PRIVATE DATE MEMBERS
   ...
   double precision, dimension(:), allocatable :: wc,Cobs,Qsour
!
!  From a namelist
   logical:: sedi_calc=.true.
   logical:: sedi_dens=.true.
   double precision:: rho_sed=2650.
   double precision:: size=62.5e-6
   double precision:: init_conc=0.
   integer:: adv_method=1
   double precision:: cnpar=0.5
   integer:: Bcup=1
   double precision:: Cup=0.
   double precision:: CbObsDt=600.
   integer:: z0bMethod
   double precision:: Cdw
   double precision:: ustarc,gs
   integer:: Bcdw=2
   integer:: out_unit
!
!----------------------------------------------------------------------
...
98 write(0,*) '       ', 'I could not open sediment.inp'
   write(0,*) '       ', 'Ill continue but set sedi_calc to false.'
   write(0,*) '       ', 'If thats not what you want you have to supply sediment.inp'
   write(0,*) '       ', 'See the Rouse example on www.gotm.net for a working sediment.inp'
   sedi_calc = .false.
   return
99 write(0,*) 'FATAL ERROR: ', 'I could not read sediment.inp'
   stop 'init_sediment'
...
! !DEFINED PARAMETERS:
   double precision, parameter                 :: g1=1.56E-3
   double precision, parameter                 :: a0=26.3
   double precision, parameter                 :: sigma=0.7963
!
! !REVISION HISTORY:
!  Original author(s): Karsten Bolding & Hans Burchard
!
...
!
! !LOCAL VARIABLES:
   double precision                  :: CBott,Cbalg
   double precision                  :: y,z,ya,za
   double precision                  :: dcdz,rho_mean
   double precision                  :: rho(0:nlev)
   double precision,save             :: Cb
   double precision                  :: RelaxT(0:nlev)
   integer                   :: i,flag
...
!BOC
   select case (out_fmt)
      ...
      case (2)
         ...
         iret = store_data(ncid,sedi_id,5,n,array=C)
