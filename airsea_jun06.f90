...
!BOC
   ...
   if((wind_error.NE.0.0).AND.(ABS(wind_error-0.25).GT.0.0001).AND.(ABS(wind_error+0.25).GT.0.0001)) then
      if((sqrt(wx_obs*wx_obs+wy_obs*wy_obs).GE.6).AND.(wind_error.GT.0)) then
         wind_error=0.0
         border=2
      else if((wind_error.GT.-1+6/sqrt(wx_obs*wx_obs+wy_obs*wy_obs)).AND.(sqrt(wx_obs*wx_obs+wy_obs*wy_obs).LT.6)) then
         wind_error=-1+6/sqrt(wx_obs*wx_obs+wy_obs*wy_obs)
         border=3
      else if(wind_error.LE.-1) then
         wind_error=-1
         border=4
...
!
! !USES:
   use meanflow, only                : T,cost_fctn,sign
...
! !LOCAL VARIABLES:
   ...
   double precision, save            :: obs1(1),obs2(1)=0.,obs3(1),obs4(1)=0.
...
if(sst_method==2) then
   ...
   if(time_diff(sst_jul2,sst_secs2,jul,secs) .lt. 0) then
      ...
      !Assimilate SST observations at observation time
           ...
           PRINT*,obs1(1),'OSTIA'
           call assimilate_satellite_obs(sst,T(1:150))
      !END SP
...
if(sst_method2==2) then 
   ...
   if(time_diff(sst2_jul2,sst2_secs2,jul,secs) .lt. 0) then
      do
         ...
         obs3 = obs4
         call read_obs(sst_unit2,yy2,mm2,dd2,hh2,min2,ss2,1,obs4,rc)
      ...
      if(first2) then
         ...
         PRINT*,obs3(1),'SEVIRI'
         sign=sign+(T(150)-obs3(1))
         cost_fctn=cost_fctn+(T(150)-obs3(1))**2
      end if
...
!EOC
!-----------------------------------------------------------------------
     subroutine assimilate_satellite_obs(sst,T)
