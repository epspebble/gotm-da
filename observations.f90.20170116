...
module observations
  ...
  integer            ::start_lat=0,end_lat=0
  integer            ::start_lon=0,end_lon=0
  ...
  !W-H. Tse 2016/09/10
  ! Original buffer size too small for including more columns or digits.
...
contains
  ...
  subroutine init_observations(namlst,fn,julday,secs,depth,nlev,z,h,latitude,longitude)
    ...
    case (12)
       open(extinct_unit,file=extinct_file,status='unknown',err=105)
       write(0,*) '       ', 'Reading chlorophyll data from:'
       write(0,*) '           ', trim(extinct_file)
       DO i=1,((end_lon-start_lon)+1)*(nint(latitude)-start_lat)+(nint(longitude)-start_lon)+1
          READ(extinct_unit,FMT='(3X,E13.11)') chlo
       END DO
       !SP 22/02/06  The parameterisation holds for values of chlophyll between 0.03 and 3.0 
       IF(chlo.gt.3.0) THEN
          chlo=3.0
       END IF
...
109 write(0,*) 'FATAL ERROR: ', 'Unable to open "',trim(e_prof_file),'" for reading'
  ...
  subroutine read_obs(unit,yy,mm,dd,hh,min,ss,N,obs,ierr)
    ...
    !BOC
    ierr=0
    read(unit,'(A255)',ERR=100,END=110) cbuf !Update format after changing buffer size.
