...
! !INPUT PARAMETERS:
   ...
   double precision, intent(in)      :: h(0:N),Y(0:N)
   double precision, intent(in)      :: w(0:N)
...
!
! !OUTPUT PARAMETERS:
   double precision, intent(out):: Qadv(0:N)
!
! !REVISION HISTORY: 
...
!
! !LOCAL VARIABLES:
   double precision                  :: Cint(0:N),Clow(0:N)
   double precision                  :: Chig(0:N),Ci(0:N)   
   double precision                  :: rp(0:N),rm(0:N) 
   double precision                  :: Cd,Cu,Cuu,ww,CMin,CMax,Cl
   double precision                  :: UppFlux,LowFlux,fac     
   integer                   :: i 
! 
!EOP
!-----------------------------------------------------------------------
...
!
   if (Method.eq.3) then
      if (surf_flux.or.bott_flux) then
         write(0,*) 'Flux corrected transport in combination with '
         write(0,*) 'surface fluxes is not coded yet, change to Method 1 or 2. '
