...
contains
  ...
  subroutine init_time(MinN,MaxN)
    ...
    !
    MinN = 1
    write(0,*) '       ', 'Time step:      ',timestep,' seconds'
    write(0,*) '       ', 'Time format:    ',timefmt
    select case (timefmt)
    case (1)
       HasRealTime=.false.
       write(0,*) '       ', '# of timesteps: ',MaxN
       start='2000-01-01 00:00:00'
       write(0,*) '       ', 'Fake start:     ',start
    case (2)
       write(0,*) '       ', 'Start:          ',start
       write(0,*) '       ', 'Stop:           ',stop
       ...
       write(0,*) '        ==> ',ndays,' day(s) and ',nsecs,' seconds ==> ',MaxN,' micro time steps' 
    case (3)
       write(0,*) '       ', 'Start:          ',start
       write(0,*) '       ', '# of timesteps: ',MaxN
       ...
       write(0,*) '       ', 'Stop:           ',stop
    case default
       write(0,*) 'Fatal error: A non valid input format has been chosen'
       stop 'init_time'
    end select
  ...
  integer function tz(lon)

    ! DESCRIPTION
    ! This function returns the theoretical time zone depending only on
    ! longitude.
    !
    ! USES
    IMPLICIT NONE
  ...
  subroutine UTC_to_local(jul,secs,lon,ljul,lsecs)
    ...
    ! Adjust the number of seconds since midnight by theoretical timezone.
    lsecs = secs + tz(lon)*3600
