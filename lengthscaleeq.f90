...
! !INPUT PARAMETERS:
   integer, intent(in)                 :: n
   double precision, intent(in)                :: dt,depth
   double precision, intent(in)                :: u_taus,u_taub,z0s,z0b
   double precision, intent(in)                :: h(0:N)
   double precision, intent(in)                :: P(0:N),B(0:N),NN(0:N)
!
! !DEFINED PARAMETERS:
!  boundary conditions 
...
! !LOCAL VARIABLES:
   integer                   :: i
   double precision                  :: avh(0:N),q2l(0:N),q3(0:N)
   double precision                  :: pminus(0:N),pplus(0:N)
   double precision                  :: Lz(0:N)
   double precision                  :: prod,buoyan,diss,ds,db,Lcrit
   double precision                  :: bc_tmp
   double precision                  :: ki,l_min
!
!------------------------------------------------------------------------
!BOC
