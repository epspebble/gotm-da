...
module airsea
  ...
  use time, only: UTC_to_local, tz, fractional_year !WT new functions
  ...
  !WT 20180829 For compatibility with gfortran. Added an extra check if the extinction method and albedo method uses the same file.
  use observations, only: extinct_method, extinct_unit

  !WT Move short_wave_radiation() elsewhere.
  !use solar_radiation, only: short_wave_radiation
...
999 write(0,*) 'Finished temporary hack for coolskin control.'
    ...
    case (1)
       if (extinct_method .eq. 12) then
          write(0,*) '       ', 'Ohlmann-Siegel (2000) light extinction method already selected. Skip reading the chlorophyll-a data again.'
          albedo_unit=extinct_unit
       else
          open(albedo_unit,file=albedo_file,action='read',status='old',err=100)
          write(0,*) '       ', 'Using the implicit albedo due to Ohlmann-Siegel (2000).'
          write(0,*) '       ', 'Chlorophyll-a data required for computing Ohlmann-Siegel (2000) albedo. Reading from:'
...
double precision              :: K_ir,K_vis,K1,K2
    ...
    ea = esat(airt,airp)         !Teten's returns sat. vapour pressure, at air temp., ea, in mb
    ...
    es = esat(sst,airp)        !returns saturated vapour pressure at SST, es, in mb
...
912    continue
     ...
     function esat(T,P)                                 
       ...
       !WT Originally named humidity as a subroutine.
...
!      ! WT Copied to solar.f90, to be removed.
!           !EOC
!      !-----------------------------------------------------------------------
!      !BOP
!      !
