! WT This file is part of the 'airsea' module
!
! WT 2018-08-22 'airsea' module public parameters, gotm-da version v3g
!
!    ! !PUBLIC DATA MEMBERS:
...
subroutine short_wave_radiation(jul,secs,alat,alon,swr)
  ...
  !
  ! !USES:
  !use observations, only: chlo ! WT Moved to within albedo_OS2000() and albedo_JIN2011()
  IMPLICIT NONE
  !
  ! !INPUT PARAMETERS:
  ...
  ! List of parameters used from airsea module
  ! General convenience parameters
  double precision, parameter                :: pi=3.14159265358979323846
  double precision, parameter                :: deg2rad=pi/180.
  double precision, parameter                :: rad2deg=180./pi

  ! Model parmeters from Rosati (88)
  double precision, parameter                :: eclips=23.439*deg2rad
  ! tau = 0.7 gives a better fit compared to 0.66 in medsea.
  double precision, parameter                :: tau=0.7  !Arab=0.74,COARE=0.63,Sub=0.7
  ...
  ! HERE
  !double precision :: albedo, cosr, sunbet, qdir_frac, qtot ! not in the list, probably a module variable
  !double precision :: wx_obs, wy_obs ! take in from airsea module?

  ! qdir, qdiff are clear-sky values of direct and diffuse irradiance
  ...
  coszen = coszen_NOAA1(jul,secs,alat,alon)
  ...
  ! deviation from the approx. spring solstice, normalized to 2*pi.
...
function albedo_PAYNE1972(coszen)
...
end function albedo_PAYNE1972

function albedo_OS2000(coszen)
  use observations, only: chlo ! WT Moved to within albedo_OS2000() and albedo_JIN2011()
  double precision, intent(in) :: coszen
  double precision, intent(out) :: albedo_OS2000

  double precision, parameter :: C1(16) = &
       (/.026,-.009,-.015,-.003,.063,.278,3.91,16.64, &
       .033,-.01,-.019,-.006,.066,.396,7.68,51.27/)
  double precision, parameter :: C2(16) = &
       (/.112,.034,-.006,-.131,-.015,-.562,-12.91,-478.28, &
       .0,.0,.0,.0,.0,.0,.0,.0/)
  double precision, parameter :: C3(16) = &
       (/.0,.0,.0,.0,.0,.0,.0,.0, &
       -.025,-.007,-.003,-.004,.006,-.027,-2.49,13.14/)
  double precision, parameter ::C4(16) = &
       (/.366,.207,.188,.169,.082,1.02,16.62,736.56, &
       .419,.231,.195,.154,.066,.886,17.81,665.19/)

  double precision para_A, Trans_1
  !-------------------------------------------HX
  !---------------------------------------- Ohlmann & Siegel (2000)
...
end function albedo_OS2000

function diffuse_radiance_ERBS1982(coszen)
!  use airsea, only: solar, gamma, I_0
  implicit None
  double precision, intent(in) :: coszen
  double precision, intent(out) :: diffuse_radiance_ERBS1982
  double precision :: f_dir ! alias

  double precision :: EI, k_t 
  ...
  diffuse_radiance_ERBS1982 = f_dif
end function diffuse_radiance_ERBS1982
  
function albedo_JIN2011(coszen)
  use observations, only: chlo
  !use airsea, only: chlo, cloud, qtot
  double precision, intent(in) :: coszen
  double precision, intent(out) :: albedo_JIN2011
  
  double precision, parameter :: n_a = 1., n_0 = 1.34 ! refractive index of air and seawater respectively
  
  double precision             :: fmiusigma,alphasdif,alphasdir,rpara,rperp,Rtotal,sigma

  ! WT/SP
  ! EI = 'extraterrestrial radiation',
  ! k_t = 'clearness index' for ERBS model code by SP,
  ! see https://plantpredict.com/algorithm/irradiance-radiation/#erbs-model for reference.
  ! f_dir, f_dif, f_dir + f_dif = 1, are fractions or direct and diffuse irradiance respectively, which are
  ! the generalized counterpart of qdir_frac and qdiff_frac found from Rosati's formula. 
  ...
  double precision             ::sigmasquare,wind,fwc,albedoe
  ...
  ! WT An alternative is to use Rosati (88) formula and put:
  !    f_dir = qdir / qtot
  !    f_dif = 1 - f_dir
  ! That should work okay for a clear day.
  
  ! Erbs's formula used here, implemented by SP and WT.  
  f_dif = diffuse_radiance_Erb(coszen)
  f_dir = 1. - f_dif

  ! WT Also replace the clear sky counterparts from Rosati's formulas, which
  ! in turn will influence the cool skin parametrization
  qdir_frac = f_dir
  qdiff_frac = f_dif
  ...
  if (qtot.gt.0) then
     if (coszen==0.0) then
        ...
        !       albedo = f_dir*alphasdir+f_dif*alphasdif+0.006  !?
  ...
  !---------------------------------------- end Jin et al. (2011)
end function albedo_JIN2011

function surface_albedo(coszen)
  double precision, intent(in) :: coszen
  double precision, intent(out) :: surface_albedo
  double precision :: albedo ! alias
  ...
  case (0)
     albedo = albedo_PAYNE1972(coszen)
  case (1)
     albedo = albedo_OS2000(coszen)
  case (2)
     albedo = albedo_JIN2011(coszen)
...
function coszen_NOAA1(jul,secs,alat,alon)
  use time, only: fractional_year, tz
  implicit None
  integer, intent(in) :: jul, secs
  double precision, intent(in) :: alat, alon
  double precision, intent(out) :: coszen_NOAA1
  double precision ::  lat, lon
  integer :: yrdays, ljul, lsecs
  ...
  coszen_NOAA1 =sin(alat)*sin(decl)+cos(alat)*cos(decl)*cos(ha) ! ha, decl not needed anymore from now on
