...
!  private data memebers
   ...
   double precision                  :: T0,S0,p0,dtr0,dsr0
...
!-----------------------------------------------------------------------
!BOC
   write(0,*) '   ', 'init_eqstate'
   if(present(namlst)) then
      read(namlst,nml=eqstate,err=80)
   end if
   return
   80 write(0,*) 'FATAL ERROR: ', 'I could not read "eqstate" namelist'
   stop 'init_eqstate'
...
!
! !INTERFACE:
   double precision function eqstate1(S,T,p,g,rho_0) 
!
! !DESCRIPTION:
...
!
! !INPUT PARAMETERS:
   double precision,intent(in)                 :: S,T,p
   double precision,optional,intent(in)        :: g,rho_0
!
! !REVISION HISTORY:
!  Original author(s): Hans Burchard \& Karsten Bolding
!
...
! !LOCAL VARIABLES:
   double precision                  :: x
   double precision, save            :: rh0,dtr,dsr
   double precision                  :: dTT,dSS
   logical                   :: press
...
!
! !INTERFACE:
   double precision function unesco(S,T,p,UNPress) 
!
! !DESCRIPTION:
...
! !LOCAL VARIABLES:
   ...
   double precision                  :: T2,T3,T4,T5,S15,S2,S3,p2 
!
!-----------------------------------------------------------------------
!BOC
   T2 = T*T
